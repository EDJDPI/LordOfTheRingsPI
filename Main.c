...
void TakeTurn(char board[ROWS][COLUMNS], struct Building gondorBuildings[], struct Building mordorBuildings[], struct Unit gondorUnits[], struct Unit mordorUnits[], struct Mines gondorMines[], struct Mines mordorMines[], struct CastarCoins *coins, Heros Gandalf[], struct PlacedEntity **placedHeroes, int *placedHeroCount)
{
	...
	while (endTurn != 1)
	{
		...
		break;
		case 8:
		SaveGame(board, gondorBuildings, mordorBuildings, gondorUnits, mordorUnits, gondorMines, mordorMines, coins, Gandalf, placedHeroes, placedHeroCount);
		exit(0);
		default:
...
void SaveGame(char board[ROWS][COLUMNS], struct Building gondorBuildings[], struct Building mordorBuildings[], struct Unit gondorUnits[], struct Unit mordorUnits[], struct Mines gondorMines[], struct Mines mordorMines[], struct CastarCoins *coins, Heros Gandalf[], struct PlacedEntity **placedHeroes, int *placedHeroCount) {
    FILE *file;
    ...
    // Save board state
    fwrite(board, sizeof(char), ROWS * COLUMNS, file);

    // Save other game state variables
    fwrite(gondorBuildings, sizeof(struct Building), 1, file);
    fwrite(mordorBuildings, sizeof(struct Building), 1, file);
    fwrite(gondorUnits, sizeof(struct Unit), 1, file);
    fwrite(mordorUnits, sizeof(struct Unit), 1, file);
    fwrite(gondorMines, sizeof(struct Mines), 1, file);
    fwrite(mordorMines, sizeof(struct Mines), 1, file);
    fwrite(coins, sizeof(struct CastarCoins), 1, file);
    fwrite(Gandalf, sizeof(Heros), 1, file);
    fwrite(placedHeroes, sizeof(struct PlacedEntity), *placedHeroCount, file);

    // Save placedHeroCount
    fwrite(placedHeroCount, sizeof(int), 1, file);
...
void PlaceBuilding(char board[ROWS][COLUMNS], struct Building gondorBuildings[], struct Building mordorBuildings[], struct Mines gondorMines[], struct Mines mordorMines[], struct CastarCoins *coins, struct PlacedEntity **placedHeroes, int *placedHeroCount)
{
	...
	if (choice >= 1 && choice <= 5)
	{
		...
		if (currentPlayer == 1)
			{
			if (coins->gon > building->cost)
			{			
				coins->gon -= building->cost;
				PlaceUnitHelper(board, building, placedHeroes, placedHeroCount);
			}
			else
			{
...
void PlaceHero(char board[ROWS][COLUMNS], Heros *hero, struct PlacedEntity **placedHeroes, int *placedHeroCount, struct CastarCoins *coins)
{
	...
	if (choice == 1)
	{
		if ((currentPlayer == 1 && heroGon < hero->uPT) || (currentPlayer == 2 && heroMon < hero->uPT))
		{
			if (currentPlayer == 1)
			{
				if (coins->gon > hero->character.cost)
					{			
			...
			{
				if (coins->mor > hero->character.cost)
					{			
